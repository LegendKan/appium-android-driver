...
import {isOnPlatform, WetestHelper} from './platform';
...
// use platform screenshot api when run on wetest.qq.com.
commands.getScreenshot = async function () {
  // check wether on platform
  if (isOnPlatform){
    try {
      let helper = WetestHelper.getPlatformHelper();
      let result = await helper.takeScreenshot();
      if (result == undefined || result.length == 0) {
          result = "platform take screenshot success";
      }
      // uniform to base64
      result = new Buffer(result.toString()).toString('base64');
      return result;
    } catch (err){
      log.warn(`Platform take screenshot error: ${err}`);
    }
  }
